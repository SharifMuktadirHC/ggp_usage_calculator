...
{%- assign ggp_brand_namespace = 'custom' -%}
{%- assign ggp_brand_key = 'engine' -%}
...
| `ggp_brand_namespace` / `ggp_brand_key` | Metafield shown as "brand" on suggestion cards. Currently set to `custom.engine`. |
...
- **Product suggestions** — pulled live from your `generator` collection via the metafields above; shows name, kVA, brand, and links straight to the product page
...
- If `generator` isn't the correct collection handle, or `custom.prime_power` / `custom.engine` aren't the right metafields, the "Recommended Gogopower Generators" section will simply show no matches — update the config block and re-save.
